...
class PPOPolicy(BasePolicy):
    ...
    def act_high(self, state, mask):
        prob = self.model.predict(state, mask, return_prob=True)
        goals = np.argsort(prob, axis=1)[:, -2:]
        goal = np.array([self.model.goal_to_idx[tuple(sorted(g))] for g in goals])
        
        return goal
        
    def _build_nets(self):
        num_goals = self.model.num_goals
        ...
        self.goal = tf.placeholder(tf.int32, shape=[None], name='goal')
        ...
        with tf.variable_scope('actor'):
            inp = tf.concat([embed, tf.one_hot(self.goal, num_goals)], axis=1)
        ...
        for _ in range(num_episodes):
            ...
            while not np.all(done):
                ...
                for _ in range(self.model.num_steps_per_goal):
                    ...
                    traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), prob.copy(), goal.copy(), g.copy(), a.copy(), action.copy())]
                    s, m = s_next, m_next
            prob = self.model.predict(s, m, return_prob=True)
            traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), prob.copy(), goal.copy(), g.copy(), a.copy(), action.copy())]
