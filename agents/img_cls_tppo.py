...
class PPOPolicy(BasePolicy):
    def act(self, state, mask, aux, avail, done, rand=False):
        if rand:
            # random action for warm start
            avail = np.concatenate([avail, np.ones([avail.shape[0], 1], dtype=np.bool)], axis=1)
            ...
            action = np.array([np.random.choice(self.model.num_actions+1, p=p) for p in prob], dtype=np.int32)
    ...
    def _build_nets(self):
        ...
        with tf.variable_scope('actor'):
            actor_layers = self.hps.actor_layers + [num_actions+1] # for terminal action
            actor_logits = dense(embed, actor_layers, output='', name='actor')
            inf_tensor = -tf.ones_like(actor_logits) * np.inf
            avail = tf.concat([self.avail, tf.ones([tf.shape(self.avail)[0], 1], dtype=tf.bool)], axis=1)
            self.actor_logits = tf.where(avail, actor_logits, inf_tensor)
    ...
    def _generate_rollout(self, buffer, rand=False):
        ...
        while not np.all(done):
            ...
            action = self.act(s, m, aux, avail, done, rand=rand)
        ...
        n_rec = 0
        for j in range(B):
            done = [f[j] for f in dones]
            Tj = np.min(np.where(done)[0]) + 1
            n_rec += Tj
            state = [s[j] for s in states][:Tj]
            mask = [m[j] for m in masks][:Tj]
            auxiliary = [x[j] for x in auxiliaries][:Tj]
            avail = [a[j] for a in avails][:Tj]
            action = [a[j] for a in actions][:Tj]
            reward = [r[j] for r in rewards][:Tj]
            logp = [l[j] for l in logps][:Tj]
            vpred = [v[j] for v in vpreds][:Tj]
            
            # Compute TD errors
            td_errors = [reward[t] + self.hps.gamma * vpred[t+1] - vpred[t] for t in range(Tj-1)]
            td_errors += [reward[Tj-1] + self.hps.gamma * 0.0 - vpred[Tj-1]]
            ...
            v_target = np.array(advs) + np.array(vpred)

            # add into memory buffer
            for t in range(Tj):
    ...
    def evaluate(self, load=True, num_episodes=10):
        ...
        for _ in range(num_episodes):
            ...
            while not np.all(done):
                aux = self.model.get_auxiliary(s, m)
                avail = self.model.get_availability(s, m)
                action = self.act(s, m, aux, avail, done)
                s_next, m_next, re, done = self.env.step(action)
                ri = self.model.get_reward(s, m, action, s_next, m_next, done, self.env.y)
                r = re + ri
                episode_reward += r
            ...
            metrics['num_acquisitions'].append(m.sum(axis=(1,2,3)))
