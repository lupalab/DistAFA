    ...
    def _generate_rollout(self, buffer, rand=False):
        ...
        while not np.all(done):
            ...
            for _ in range(num_steps_per_goal):
                ...
                ri = self.model.get_intrinsic_reward(s, m, goal, action, s_next, m_next, done, self.env.x)
                rm = self.model.get_meta_reward(s, m, action, s_next, m_next, done, self.env.x)
        ...
        for _ in range(num_episodes):
            ...
            while not np.all(done):
                ...
                for _ in range(self.model.num_steps_per_goal):
                    ...
                    traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), pred.copy(), goal.copy(), g.copy(), a.copy(), action.copy())]
                    s, m = s_next, m_next
            pred = self.model.predict(s, m)
            traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), pred.copy(), goal.copy(), g.copy(), a.copy(), action.copy())]
            ...
            pred = self.model.predict(s, m)
            mse = np.mean(np.square((pred - self.env.x) / 255.), axis=(1,2,3))
