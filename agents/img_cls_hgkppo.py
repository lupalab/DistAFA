...
class PPOPolicy(BasePolicy):
    def act_low(self, state, mask, aux, goal, avail_g, avail_a, rand=False):
    ...
    def _build_nets(self):
        ...
        with tf.variable_scope('actor'):
            ...
            inp = dense(inp, self.hps.actor_layers, name='actor')
            # group
    ...
    def _generate_rollout(self, buffer, rand=False):
        ...
        while not np.all(done):
            ...
            for _ in range(num_steps_per_goal):
                ...
                g, a = self.act_low(s, m, aux, goal, avail_g, avail_a, rand=rand)
    ...
    def run(self):
        BufferRecord = namedtuple('BufferRecord', 
            ['state', 'mask', 'aux', 'avail_group', 'avail_action', 'goal', 'group', 'action', 'old_logp', 'v_target', 'adv'])
        ...
        for _ in range(num_episodes):
            ...
            while not np.all(done):
                ...
                for _ in range(self.model.num_steps_per_goal):
                    aux = self.model.get_auxiliary(s, m)
                    avail_g, avail_a = self.model.get_availability(s, m)
                    g, a = self.act_low(s, m, aux, goal, avail_g, avail_a)
                    action = self.model.get_action(g, a)
                    s_next, m_next, re, done = self.env.step(action)
                    ri = self.model.get_intrinsic_reward(s, m, goal, action, s_next, m_next, done, self.env.y)
                    rm = self.model.get_meta_reward(s, m, action, s_next, m_next, done, self.env.y)
                    ...
                    traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), prob.copy(), goal.copy(), g.copy(), a.copy(), action.copy(), posterior.copy())]
                    s, m = s_next, m_next
            prob = self.model.predict(s, m, return_prob=True)
