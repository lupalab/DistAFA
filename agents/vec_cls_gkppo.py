...
class PPOPolicy(BasePolicy):
    def act_low(self, state, mask, aux, goal, avail, rand=False):
    ...
    def act_high(self, state, mask, return_prob=False):
        prob = self.model.cluster(state, mask, return_prob=True)
        goal = np.argsort(prob, axis=1)[:, -self.hps.k_clusters:]
        goal = self.model.goal_representation(goal)

        if return_prob: return goal, prob
    ...
    def _build_nets(self):
        ...
        self.goal = tf.placeholder(tf.float32, shape=[None, num_clusters], name='goal')
        ...
        with tf.variable_scope('actor'):
            inp = tf.concat([embed, self.goal], axis=1)
            actor_layers = self.hps.actor_layers + [num_actions]
            actor_logits = dense(inp, actor_layers, output='', name='actor')
        ...
        with tf.variable_scope('critic'):
            inp = tf.concat([embed, self.goal], axis=1)
            inp = dense(inp, self.hps.critic_layers, name='critic')
    ...
    def _generate_rollout(self, buffer, rand=False):
        num_steps_per_goal = self.model.num_steps_per_goal        
        n_rec = 0
        ...
        while not np.all(done):
            goal = self.act_high(s, m)
            for _ in range(num_steps_per_goal):
                aux = self.model.get_auxiliary(s, m)
                avail = self.model.get_availability(s, m)
                action = self.act_low(s, m, aux, goal, avail, rand=rand)
                s_next, m_next, re, done = self.env.step(action)
                ri = self.model.get_intrinsic_reward(s, m, goal, action, s_next, m_next, done, self.env.y)
                rm = self.model.get_meta_reward(s, m, action, s_next, m_next, done, self.env.y)
                r = re + ri * self.hps.intrinsic_reward_coeff + rm * self.hps.meta_reward_coeff
    ...
    def run(self):
        BufferRecord = namedtuple('BufferRecord', 
            ['state', 'mask', 'aux', 'avail', 'goal', 'action', 'old_logp', 'v_target', 'adv'])
        ...
        for n_iter in range(self.hps.n_iters):
            ...
            if hasattr(self.hps, 'warmup_iters') and n_iter <self.hps.warmup_iters:
                rand = True
            if hasattr(self.hps, 'rand_epsilon') and np.random.rand() < self.hps.rand_epsilon:
                rand = True
    ...
    def evaluate(self, load=True, num_episodes=10):
        ...
        for _ in range(num_episodes):
            ...
            traj = []
            while not np.all(done):
                goal, posterior = self.act_high(s, m, return_prob=True)
                for _ in range(self.model.num_steps_per_goal):
                    aux = self.model.get_auxiliary(s, m)
                    avail = self.model.get_availability(s, m)
                    action = self.act_low(s, m, aux, goal, avail)
                    s_next, m_next, re, done = self.env.step(action)
                    ri = self.model.get_intrinsic_reward(s, m, goal, action, s_next, m_next, done, self.env.y)
                    rm = self.model.get_meta_reward(s, m, action, s_next, m_next, done, self.env.y)
                    r = re + ri * self.hps.intrinsic_reward_coeff + rm * self.hps.meta_reward_coeff
                    ...
                    traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), prob.copy(), goal.copy(), action.copy(), posterior.copy())]
                    s, m = s_next, m_next
            prob = self.model.predict(s, m, return_prob=True)
            posterior = self.model.cluster(s, m, return_prob=True)
            traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), prob.copy(), goal.copy(), action.copy(), posterior.copy())]
