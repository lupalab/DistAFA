...
class PPOPolicy(BasePolicy):
    ...
    def _generate_rollout(self, buffer, rand=False):
        ...
        while not np.all(done):
            ...
            ri = self.model.get_reward(s, m, action, s_next, m_next, done, self.env.x)
            r = re + ri
            if self.hps.detector_reward_coef > 0:
                rd = self.detector.get_reward(s, m, action, s_next, m_next, done, self.env.x)
    ...
    def evaluate(self, load=True, num_episodes=10):
        ...
        for _ in range(num_episodes):
            ...
            while not np.all(done):
                ...
                traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), pred.copy(), g.copy(), a.copy(), action.copy())]
                s, m = s_next, m_next
            pred = self.model.predict(s, m)
            traj += [(self.env.x.copy(), self.env.y.copy(), s.copy(), m.copy(), pred.copy(), g.copy(), a.copy(), action.copy())]
    ...
    def plot(self, num_episodes=10):
        ...
        for i in range(num_episodes):
            ...
            while not np.all(done):
                ...
                save_image(pred.astype(np.uint8), f'{sdir}/p{step}.png')
                pickle.dump(pred, open(f'{sdir}/p{step}.pkl','wb'))
                step += 1
