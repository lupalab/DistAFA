...
class NCSN1d(nn.Module):
    def __init__(self, config):
        ...
        self.begin_linear = nn.Linear(config.data.dimension*2, ngf)
        self.normalizer = self.norm(ngf, self.num_classes)
        self.end_linear = nn.Linear(ngf, config.data.dimension)

        self.res1 = nn.ModuleList([
            ConditionalResidualBlock1d(self.ngf, self.ngf, self.num_classes, 
        ...
        self.res2 = nn.ModuleList([
            ConditionalResidualBlock1d(self.ngf, 2 * self.ngf, self.num_classes, 
                                       act=act, normalization=self.norm),
            ConditionalResidualBlock1d(2 * self.ngf, 2 * self.ngf, self.num_classes,
                                       act=act, normalization=self.norm)
        ])
        ...
        self.res4 = nn.ModuleList([
            ConditionalResidualBlock1d(2 * self.ngf, 2 * self.ngf, self.num_classes, 
                                       act=act, normalization=self.norm),
            ConditionalResidualBlock1d(2 * self.ngf, self.ngf, self.num_classes,
                                       act=act, normalization=self.norm)
        ])
    ...
    def forward(self, x, m, y):
        ...
        output = self.normalizer(layer4, y)
...
class NCSN(nn.Module):
    def __init__(self, config):
        ...
        self.begin_conv = nn.Conv2d(config.data.channels+1, ngf, 3, stride=1, padding=1)
