...
from models import get_sigmas, get_model
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for i, (X, y) in enumerate(dataloader):
                ...
                loss = anneal_dsm_score_estimation(score, X, mask, sigmas, None,
                ...
                if step % 100 == 0:
                    ...
                    with torch.no_grad():
                        test_dsm_loss = anneal_dsm_score_estimation(test_score, test_X, test_mask, sigmas, None,
                                                                    self.config.training.anneal_power,
                                                                    hook=test_hook)
                ...
                if step % self.config.training.snapshot_freq == 0:
                    ...
                    if self.config.training.snapshot_sampling:
                        ...
                        init_masks = torch.ones(36, 1, self.config.data.image_size, self.config.data.image_size,
                                                device=self.config.device)

                        all_samples = anneal_Langevin_dynamics(init_samples, init_masks, test_score, sigmas.cpu().numpy(),
    ...
    def sample(self):
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.joint:
                ...
                init_masks = torch.ones(self.config.sampling.batch_size, 1, self.config.data.image_size, self.config.data.image_size,
                                                device=self.config.device)
                all_samples = anneal_Langevin_dynamics(init_samples, init_masks, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr,
                                                        final_only=True, verbose=True,
                                                        denoise=self.config.sampling.denoise)
                sample = all_samples[-1].view(all_samples[-1].shape[0], self.config.data.channels,
                                            ...
                                            self.config.data.image_size)
                sample = inverse_data_transform(self.config, sample)
                torch.save(sample, os.path.join(self.args.image_folder, 'joint_{}.pth'.format(self.config.sampling.ckpt_id)))
                torch.save(init_masks, os.path.join(self.args.image_folder, 'joint_masks_{}.pth'.format(self.config.sampling.ckpt_id)))
                
                if self.config.data.channels == 1:
                    sample = sample.expand([-1,3,-1,-1])
                sample = sample * init_masks + torch.ones_like(sample) / 255. * torch.tensor([60.,250.,200.]).view(1,3,1,1) * (1-init_masks)

                image_grid = make_grid(sample[:width*width], width)
                save_image(image_grid, os.path.join(self.args.image_folder, 'joint_{}.png'.format(self.config.sampling.ckpt_id)))
                
            elif self.config.sampling.marginal:
                data_iter = iter(dataloader)
                refer_images, _ = next(data_iter)
                refer_images = refer_images.to(self.config.device)
                width = int(np.sqrt(self.config.sampling.batch_size))
                ...
                mask_fn = get_mask(self.config)
                init_masks = mask_fn(init_samples)
                all_samples = anneal_Langevin_dynamics(init_samples, init_masks, score, sigmas,
                                                        self.config.sampling.n_steps_each,
                                                        self.config.sampling.step_lr,
                                                        final_only=True, verbose=True,
                ...
                sample = all_samples[-1].view(all_samples[-1].shape[0], self.config.data.channels,
                                            ...
                                            self.config.data.image_size)
                sample = inverse_data_transform(self.config, sample)
                torch.save(sample, os.path.join(self.args.image_folder, 'marginal_{}.pth'.format(self.config.sampling.ckpt_id)))
                torch.save(init_masks, os.path.join(self.args.image_folder, 'marginal_masks_{}.pth'.format(self.config.sampling.ckpt_id)))
                ...
                image_grid = make_grid(sample[:width*width], width)
                save_image(image_grid, os.path.join(self.args.image_folder, 'marginal_{}.png'.format(self.config.sampling.ckpt_id)))
            
            elif self.config.sampling.inpainting:
                data_iter = iter(dataloader)
                refer_images, _ = next(data_iter)
                refer_images = refer_images.to(self.config.device)
                width = int(np.sqrt(self.config.sampling.batch_size))
