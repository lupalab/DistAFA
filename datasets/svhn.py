...
image_shape = [32, 32, 3]
data_path = '/data/svhn/'
train_loaded = loadmat(f'{data_path}/test_32x32.mat')
train_x = np.transpose(train_loaded['X'], [3,0,1,2])
train_y = train_loaded['y'].astype(np.int64).squeeze()
np.place(train_y, train_y == 10, 0)
test_loaded = loadmat(f'{data_path}/train_32x32.mat')
test_x = np.transpose(test_loaded['X'], [3,0,1,2])
test_y = test_loaded['y'].astype(np.int64).squeeze()
np.place(test_y, test_y == 10, 0)
...
aug_fn = AutoAugment(AutoAugmentPolicy.SVHN)
