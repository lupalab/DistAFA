...
def conditional_prior(y, n_classes, image_shape):
    with tf.variable_scope('prior', reuse=tf.AUTO_REUSE):
        B = tf.shape(y)[0]
        H,W,C = image_shape

        h1 = tf.zeros([B,H,W,C*2])
        h1 = tf.layers.conv2d(h1, C*2, 3, 1, padding='same', name='h1', kernel_initializer=tf.zeros_initializer())
        h1_logs = tf.get_variable('h1/logs', [C*2], initializer=tf.zeros_initializer())
        h1 *= tf.exp(h1_logs * 3.)

        y_onehot = tf.one_hot(y, n_classes)
        h2 = tf.layers.dense(y_onehot, C*2, name='h2', kernel_initializer=tf.zeros_initializer())
        h2_logs = tf.get_variable('h2/logs', [C*2], initializer=tf.zeros_initializer())
        h2 *= tf.exp(h2_logs * 3.)

        h = h1 + tf.reshape(h2, [B, 1, 1, C*2])

        mean, logsd = h[:,:,:,:C], h[:,:,:,C:]

    return mean, logsd

def conditional_normal_ll(z, y, n_classes, image_shape):
    mean, logsd = conditional_prior(y, n_classes, image_shape)
    logp = -0.5 * (numpy.log(2 * numpy.pi) + 2. * logsd + (z - mean) ** 2 / tf.exp(2. * logsd))

    return logp

def conditional_normal_sample(y, n_classes, image_shape):
    mean, logsd = conditional_prior(y, n_classes, image_shape)
    z = tf.random_normal([tf.shape(y)[0]]+image_shape)
    
    return z * tf.exp(logsd) + mean

def conditional_normal_mean(y, n_classes, image_shape):
    mean, logsd = conditional_prior(y, n_classes, image_shape)
