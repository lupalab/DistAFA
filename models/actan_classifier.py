...
class FlowClassifier(BaseModel):
    ...
    def classify(self, x, b, m):
        ...
        x = tf.reshape(x, [B*N, self.hps.dimension])
        b = tf.tile(tf.expand_dims(b, axis=1), [1,N,1])
        b = tf.reshape(b, [B*N, self.hps.dimension])
        m = tf.tile(tf.expand_dims(m, axis=1), [1,N,1])
        m = tf.reshape(m, [B*N, self.hps.dimension])
    ...
    def build_net(self):
        with tf.variable_scope('flow_classifier', reuse=tf.AUTO_REUSE):
            ...
            num_dimensions = tf.reduce_sum(self.m * (1-self.b), axis=-1) + 1e-6
